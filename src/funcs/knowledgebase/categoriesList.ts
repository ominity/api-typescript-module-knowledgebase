...
} from "../../models/knowledgebase/category.js";
import {
    ListCategoriesRequest,
    ListCategoriesRequest$outboundSchema,
} from "../../models/operations/knowledgebase-categories.js";
...
export function categoriesList(
    client: ClientSDK,
    request?: ListCategoriesRequest | undefined,
...
> {
    const parsed = safeParse(
        request,
        (value) => ListCategoriesRequest$outboundSchema.parse(value),
        "Input validation failed",
    );
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value as ListCategoriesRequest;
    const body = null;

    const path = "/modules/knowledgebase/categories";
    ...
    const context = {
        ...
        operationID: "modules.knowledgebase.categories.list",
        oAuth2Scopes: null,
    ...
    const [result] = await M.match<
        ...
        | SDKValidationError
    >(
        M.json(200, KnowledgebaseCategoriesListResponse$inboundSchema, {
