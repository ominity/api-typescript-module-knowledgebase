...
} from "../../models/knowledgebase/tag.js";
import {
    ListTagsRequest,
    ListTagsRequest$outboundSchema,
} from "../../models/operations/knowledgebase-tags.js";
...
export function tagsList(
    client: ClientSDK,
    request?: ListTagsRequest | undefined,
...
> {
    const parsed = safeParse(
        request,
        (value) => ListTagsRequest$outboundSchema.parse(value),
        "Input validation failed",
    );
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value as ListTagsRequest;
    const body = null;

    const path = "/modules/knowledgebase/tags";
    ...
    const context = {
        ...
        operationID: "modules.knowledgebase.tags.list",
        oAuth2Scopes: null,
    ...
    const [result] = await M.match<
        ...
        | SDKValidationError
    >(
        M.json(200, KnowledgebaseTagsListResponse$inboundSchema, {
