...
import {
    GetCategoryRequest,
    GetCategoryRequest$outboundSchema,
...
> {
    const parsed = safeParse(
        request,
        (value) => GetCategoryRequest$outboundSchema.parse(value),
        "Input validation failed",
    );
    if (!parsed.ok) {
        return [parsed, { status: "invalid" }];
    }
    const payload = parsed.value as GetCategoryRequest;
    const body = null;

    const path = `/modules/knowledgebase/categories/${payload.id}`;
    ...
    const context = {
        ...
        operationID: "modules.knowledgebase.categories.get",
        oAuth2Scopes: null,
    ...
    const [result] = await M.match<
        ...
        | SDKValidationError
    >(
        M.json(200, KnowledgebaseCategory$inboundSchema, {
